...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  pending:      { label: 'En attente',   color: '#EAB308', bg: 'rgba(234,179,8,0.14)' },
  ...
  closed:       { label: 'Fermé',        color: '#9CA3AF', bg: 'rgba(156,163,175,0.14)' },
  actif:        { label: 'Actif',        color: '#22C55E', bg: 'rgba(34,197,94,0.14)' },
  inactif:      { label: 'Inactif',      color: '#9CA3AF', bg: 'rgba(156,163,175,0.14)' },
...
export function StatusBadge({ status, label, size = 'sm' }: Props) {
  ...
  const ph = size === 'md' ? 12 : 9;
  const pv = size === 'md' ? 5 : 3;
  const fs = size === 'md' ? 13 : 11;
  return (
    <View style={{ backgroundColor: cfg.bg, borderRadius: 999, paddingHorizontal: ph, paddingVertical: pv, alignSelf: 'flex-start' }}>
      <Text style={{ color: cfg.color, fontSize: fs, fontWeight: '600' }}>{label ?? cfg.label}</Text>
    </View>
  );
}
